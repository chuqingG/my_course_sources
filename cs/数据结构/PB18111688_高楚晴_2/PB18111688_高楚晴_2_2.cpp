...
int InitQueue(SqQueue &Q) {
	Q.base = (int *)malloc(MAX_NUM * sizeof(int));
	if (!Q.base)
		exit(OVERFLOW);
	Q.front = Q.rear = 0;
	return OK;
...
int EnQueue(SqQueue &Q, int e) {
	if ((Q.rear + 1) % MAX_NUM == Q.front)     //队列满
		return ERROR;
	Q.base[Q.rear] = e;
...
int DeQueue(SqQueue &Q) {               //删除队尾元素并返回其值
	int e;
	if (Q.front == Q.rear)
		return ERROR;
	e = Q.base[Q.rear - 1];
	Q.rear = (Q.rear - 1 + MAX_NUM) % MAX_NUM;
	return e;
}
Algraph *create() {   //创建表 
	int i = 0, c1, c2;
