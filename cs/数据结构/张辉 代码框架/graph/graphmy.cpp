...
void printG(Graph *g){
	...
	for(int i=0;i<g->nv;++i){
		...
		for(int j=0;j<g->v[i].degree;++j){
			...
			printf("-> %u(%d-%d) ",g->e[e].t,g->e[e].id,g->e[e].w); //与顶点g->v[i]相连的边的尾结点、边的编号、边的权值
		}
		printf("\n");
	}
}
...
int main(){
	...
	randGenGraph(5,0.8,true,false,"g3.txt",seed);  //结点个数，结点连边概率，是否有权图，是否有向图
